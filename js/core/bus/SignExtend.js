...
/**
 * SignExtend — Sign-extends a narrower bus to make it wider.
 * The sign bit (MSB of input) is replicated to fill the upper bits.
 *
 * Constructor params:
 ...
 * Ports:
 *   1 input  — M-bit bus
 *   1 output — N-bit bus (N > M, sign bit replicated)
 */
export class SignExtend extends Component {
  static label = 'Sign Extend';
  ...
  constructor(id, inputWidth = 8, outputWidth = 16) {
    ...
    super(id, 'SignExtend', 1, 1, [inputWidth], [outputWidth]);
  ...
  computeNextState() {
    ...
    // Check the sign bit (MSB of input)
    const signBit = (val.value >> (this._inputWidth - 1)) & 1;
    const signBitUnknown = (val.unknown >> (this._inputWidth - 1)) & 1;
    const signBitError = (val.error >> (this._inputWidth - 1)) & 1;
    ...
    // Propagate any error/unknown in lower bits
    const combinedValue = lowerValue | upperValue;
    const combinedUnknown = lowerUnknown | upperUnknown;
    const combinedError = lowerError | upperError;

    return {
      outputs: [new Value(this._outputWidth, combinedError, combinedUnknown, combinedValue)]
    };
  }
  ...
  render(container) {
    ...
    el.className = 'component sign-extend';
    el.style.width = `${W}px`;
