...
export class TFlipFlop extends Component {
  ...
  computeNextState() {
    ...
    if (this.bitWidth > 1) {
      // Bus-aware: T input carries a Value object
      const tValue = (t instanceof Value) ? t : Value.fromBoolean(t);
      ...
      if (this._isTriggerEdge(clk)) {
        // Toggle bits where T=1
        nextQ = nextQ.xor(tValue);
        nextNQ = nextQ.not();
      }
  ...
  setProperty(name, value) {
    if (name === 'bitWidth') {
      ...
      // Rebuild ports - T input at bus width, CLK always 1-bit
      this.inputs = [
  ...
  render(container) {
    ...
    el.className = 'component flipflop t-ff' + (this.bitWidth > 1 ? ' bus-component' : '');
    el.style.width = `${W}px`;
    ...
    body.textContent = this.bitWidth > 1 ? `T/${this.bitWidth}` : 'T';
